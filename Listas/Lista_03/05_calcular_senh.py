...
def calcular_senh(x, numero_de_iteracoes):
  """
  Calcula a aproximação de senh(x) usando a série de Taylor.
  ...
  for n in range(1, numero_de_iteracoes + 1):
    termo = (x**(2*n + 1)) / math.factorial(2*n + 1)
    soma += termo
    erro_relativo = 0.0 if termo_anterior == 0 else abs((soma - termo_anterior) / termo_anterior)
    termo_anterior = soma

  print(f"Aproximação de senh({x:.2f}) com {numero_de_iteracoes} iterações: {soma:.4f}")
...
calcular_senh(x, numero_de_iteracoes)
